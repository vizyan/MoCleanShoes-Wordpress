...
jQuery(document).ready(function(){
	...
	jQuery('input, textarea, select').not('.mts_translate_textarea, #translate_search, .nhp-opts-select-hide-below').change(function(){
        if (!changeWarn) {
    ...
    // MoneyFlow custom script
    jQuery('select[id=mts_blog_posts_layout_style], select[id=mts_index_posts_layout_style]').change(function(event) {
        if (jQuery(this).val() == '3') {
            jQuery(this).closest('tr').siblings('tr:last-child').fadeOut('slow');
        } else {
            jQuery(this).closest('tr').siblings('tr:last-child').fadeIn('slow');
        }
    }).trigger('change');
